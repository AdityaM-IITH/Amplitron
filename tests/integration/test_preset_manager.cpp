...
static bool file_exists(const std::string &path) { return std::filesystem::exists(path); }
...
static bool write_wav_mono_pcm16(const std::string &path, const std::vector<float> &samples,
    ...
    write_le32(out, static_cast<uint32_t>(sample_rate) * block_align);
...
TEST_F(PresetTest, save_creates_file) {
    ...
    bool ok = manager.save_preset(path, "Test Preset", "A test description", engine);
    ASSERT_TRUE(ok);
...
TEST_F(PresetTest, save_and_load_roundtrip) {
    ...
    if (!eq->params().empty()) {
        eq->params()[0].value =
            eq->params()[0].min_val + (eq->params()[0].max_val - eq->params()[0].min_val) * 0.75f;
    }
    ...
    bool saved = manager.save_preset(path, "Roundtrip", "roundtrip test", engine);
    ASSERT_TRUE(saved);
...
TEST_F(PresetTest, load_nonexistent_fails) {
    bool loaded = manager.load_preset("presets/does_not_exist_12345.json", engine);
    ASSERT_FALSE(loaded);
}
...
TEST_F(PresetTest, midi_mappings_roundtrip) {
    ...
    bool saved = manager.save_preset(path, "Midi Test", "Testing midi mappings", engine, mappings);
    ASSERT_TRUE(saved);
...
TEST_F(PresetTest, graph_to_json_roundtrip) {
    ...
    AudioGraph graph;
    int n1 =
        graph.add_node("Overdrive", NodeRoutingType::StandardEffect, std::make_shared<Overdrive>());
...
TEST_F(PresetTest, parallel_amp_rig_integration) {
    ...
    int amp1 = graph.add_node("Amp Sim", NodeRoutingType::StandardEffect,
                              std::make_shared<Overdrive>());  // using Overdrive as proxy
    int amp2 =
        graph.add_node("Amp Sim", NodeRoutingType::StandardEffect, std::make_shared<Overdrive>());
    ...
    bool saved = manager.save_preset(path, "Parallel Rig", "Parallel Amps", engine);
    ASSERT_TRUE(saved);
...
static void write_dummy_wav(const std::string &path) {
    ...
    const unsigned char header[44] = {'R', 'I', 'F', 'F', 40,  0,   0,   0,  'W', 'A', 'V',
                                      'E', 'f', 'm', 't', ' ', 16,  0,   0,  0,   1,   0,
                                      1,   0,   68,  172, 0,   0,   136, 88, 1,   0,   2,
                                      0,   16,  0,   'd', 'a', 't', 'a', 4,  0,   0,   0};
...
TEST_F(PresetTest, save_preset_data_invalid_path) {
    PresetData p;
    bool saved = manager.save_preset_data("/invalid_path_that_does_not_exist/preset.json", p);
    ASSERT_FALSE(saved);
}
...
TEST_F(PresetTest, save_preset_invalid_path) {
    bool saved = manager.save_preset("/invalid_path_that_does_not_exist/preset.json", "Name",
                                     "Desc", engine);
    ASSERT_FALSE(saved);
}
...
TEST(PresetJson, UnknownFieldsIgnoredOnLoad) {
    std::string json =
        R"({"format_version":1,"name":"Test","version":1,"unknown_field":42,"effects":[]})";
    PresetData preset;
...
TEST(PresetManagerIO, SavePresetDataToDirectoryFails) {
    ...
    ASSERT_FALSE(ok);
    ASSERT_TRUE(manager.get_last_error().find("Could not open file for writing") !=
                std::string::npos);
}
...
TEST(PresetJson, FromJsonExtIgnoresNonNumericParamsAndNonStringMetadata) {
    std::string json =
        R"({"format_version":1,"effects":[{"type":"TestFX","params":{"Good":1,"Bad":"x"},"metadata":{"k":123}}]})";
    PresetData p;
