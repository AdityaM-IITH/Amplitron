...
extern "C" EMSCRIPTEN_KEEPALIVE int trigger_add_splitter_node() {
    ...
    auto cmd = std::make_unique<Amplitron::AddGraphNodeCommand>(
        g_gui->audio_engine(), "Splitter", Amplitron::NodeRoutingType::Splitter, nullptr,
        ImVec2(0, 0));
    auto* raw = cmd.get();
...
extern "C" EMSCRIPTEN_KEEPALIVE int trigger_add_link(int src_pin, int dst_pin) {
    if (!g_gui) return -1;
    auto cmd =
        std::make_unique<Amplitron::AddGraphLinkCommand>(g_gui->audio_engine(), src_pin, dst_pin);
    auto* raw = cmd.get();
    g_gui->command_history().execute(std::move(cmd));
...
extern "C" EMSCRIPTEN_KEEPALIVE bool trigger_delete_last_node() {
    ...
    // Walk backwards to find the last deletable node (mirrors GUI rules: Input and Amp Sim are
    // protected)
    for (int i = static_cast<int>(nodes.size()) - 1; i >= 0; --i) {
        ...
        g_gui->command_history().execute(std::make_unique<Amplitron::RemoveGraphNodeCommand>(
...
int main(int argc, char* argv[]) {
    ...
    if (!cli_opts.exit_early && cli_opts.preset_path.empty()) {
        std::cerr << "Error: Strict headless build requires a --preset <path> argument."
                  << std::endl;
    ...
    if (cli_opts.is_headless) {
        ...
        if (!cli_opts.input_device.empty()) {
            ...
            if (match_indices.empty()) {
                std::cerr << "Warning: Could not find requested input device: '"
                          << cli_opts.input_device << "'" << std::endl;
                ...
                std::cerr << "Warning: Ambiguous input name '" << cli_opts.input_device
                          << "'. Multiple matches found:" << std::endl;
                for (int idx : match_indices) {
                    std::cerr << " -- " << devices[idx].name << std::endl;
                }
                std::cerr << "Auto-selecting the first match: " << devices[match_indices[0]].name
                          << std::endl;
                engine.set_input_device(devices[match_indices[0]].index);
            }
        }
        ...
        if (!cli_opts.output_device.empty()) {
            ...
            if (match_indices.empty()) {
                std::cerr << "Warning: Could not find requested output device: '"
                          << cli_opts.output_device << "'" << std::endl;
            } else if (match_indices.size() == 1) {
                engine.set_output_device(devices[match_indices[0]].index);
                std::cout << "Output routed to: " << devices[match_indices[0]].name << std::endl;
            } else {
                std::cerr << "Warning: Ambiguous output name '" << cli_opts.output_device
                          << "'. Multiple matches found:" << std::endl;
                for (int idx : match_indices) {
                    std::cerr << " -- " << devices[idx].name << std::endl;
                }
    ...
    if (cli_opts.is_headless) {
        ...
        std::cout << "Commands: chain, gain <val>, bypass <idx>, enable <idx>, telemetry <on/off>"
                  << std::endl;
        ...
        while (g_running) {
            ...
            for (const std::string& line : pending_commands) {
                if (line.find("gain ") == 0) {
                    ...
                    std::cout << ">> Unknown command. Available: gain <val>, bypass <idx>, enable "
                                 "<idx>, telemetry <on/off>"
                              << std::endl;
                }
            }
            ...
            if (++loop_counter >= 100) {
                if (show_telemetry.load(std::memory_order_relaxed)) {
                    ...
                    dashboard += "Peak I/O : IN " + std::to_string(in_peak) + " | OUT " +
                                 std::to_string(out_peak) + "\n";
                    dashboard += "========================================\n";
