...
bool PresetManager::save_preset_data(const std::string &filepath, const PresetData &preset) {
...
std::string PresetManager::preset_to_json_string(IAudioEngine &engine) {
    PresetData preset;
    preset.name = "Shared Preset";
    preset.description = "Generated by serverless sharing";
...
bool PresetManager::load_preset(const std::string &filepath, IAudioEngine &engine,
...
bool PresetManager::graph_from_json(const std::string &json, AudioGraph &graph) {
    PresetData preset;
    if (!from_json_ext(json, preset)) return false;

    if (preset.routing != "graph") return false;
    ...
    for (int id : nodes_to_remove) graph.remove_node(id);
    ...
    for (const auto &node : preset.nodes) {
        ...
        if (t == "splitter")
            ...
            if (!pedal) pedal = EffectFactory::instance().create(t);  // fallback
    ...
    for (const auto &link : preset.links) {
        auto parse_pin = [&](const std::string &pin_str, bool is_input) -> int {
            ...
            if (node_id_map.find(n_id) == node_id_map.end()) return -1;
        ...
        if (src_pin == -1 || dst_pin == -1) {
            std::cerr << "[Preset] Invalid link configuration, missing pin: " << link.src_pin
...
std::string PresetManager::get_last_error() const { return last_error_; }

std::string PresetManager::get_presets_directory() const { return get_presets_dir(); }
